...
const ptRoundOff = document.getElementById('ptRoundOff');
...
function renderPrintTemplate() {
  ...
  const computedPayable = total + sgstAmt + cgstAmt - discountAmount;
  const livePayable = Number(amountPayableInput.value || 0);
  const roundOff = livePayable - computedPayable;
  ...
  ptRoundOff.textContent = formatMoney(roundOff);
